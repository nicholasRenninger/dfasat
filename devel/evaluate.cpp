...
bool overlap_driven::consistent(state_merger *merger, apta_node* left, apta_node* right){
  ...
  if(left->accepting_paths >= STATE_COUNT && right->accepting_paths >= STATE_COUNT){
    int total_left = left->accepting_paths + left->num_accepting;
    int total_right = right->accepting_paths + left->num_accepting;
    ...
    for(int i = 0; i < alphabet_size; ++i){
      if(left->pos(i) >= SYMBOL_COUNT || right->pos(i) >= SYMBOL_COUNT){
        double gamma = 0.0;
        if( ((double)left->pos(i) / (double)total_left) >
            ((double)right->pos(i) / (double)total_right) )
          gamma = ((double)left->pos(i) / (double)total_left)
                - ((double)right->pos(i) / (double)total_right);
        else
          gamma = ((double)right->pos(i) / (double)total_right)
                - ((double)left->pos(i) / (double)total_left);
        if(gamma > bound){ inconsistency_found = true; return false; }
      }
    }
      if(left->num_accepting >= SYMBOL_COUNT || right->num_accepting >= SYMBOL_COUNT){
        double gamma = 0.0;
        if( ((double)left->num_accepting / (double)total_left) >
            ((double)right->num_accepting / (double)total_right) )
          gamma = ((double)left->num_accepting / (double)total_left)
                - ((double)right->num_accepting / (double)total_right);
        else
          gamma = ((double)right->num_accepting / (double)total_right)
                - ((double)left->num_accepting / (double)total_left);
        if(gamma > bound){ inconsistency_found = true; return false; }
      }
  }
  return true;
