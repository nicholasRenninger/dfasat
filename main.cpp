...
int main(int argc, const char *argv[]){
    ...
    struct poptOption optionsTable[] = {
       ...
       { "method", 'm', POPT_ARG_INT, &(param->method), 'm', "Method to use when merging states, default value 1 is random greedy (used in Stamina winner), 2 is one standard (non-random) greedy.", "integer" },
        ...
        { "sat_program", 'S', POPT_ARG_STRING, &(param->sat_program), 'S', "Relative path from binary invokation to the desired sat solver. tested to work with lingeling. e.g. -S ../lingeling/lingeling", "string" },
