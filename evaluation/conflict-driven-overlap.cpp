...
#include "conflict-driven-overlap.h"
...
REGISTER_DEF_DATATYPE(conflict_overlap_data);
REGISTER_DEF_TYPE(conflict_overlap_driven);

void conflict_overlap_data::update(evaluation_data* right){
    count_data::update(right);

    conflict_overlap_data* r = (conflict_overlap_data*)right;
...
void conflict_overlap_data::undo(evaluation_data* right){
...
void conflict_overlap_driven::update_score(state_merger *merger, apta_node* left, apta_node* right){
    conflict_overlap_data* r = (conflict_overlap_data*)right->data;
    int val = r->conflicts.size() - 2*r->undo_info.size();
    if(val < 0) val = 0;
    score = score + val;
};

int conflict_overlap_driven::compute_score(state_merger *merger, apta_node* left, apta_node* right){
    //conflict_overlap_data* r = (conflict_overlap_data*)right->data;
...
void conflict_overlap_driven::reset(state_merger *merger){
    score = 0;
...
struct conflict_compare
{
    ...
    {
        conflict_overlap_data* l = (conflict_overlap_data*)left->data;
        conflict_overlap_data* r = (conflict_overlap_data*)right->data;
...
void keep_conflicts(set<apta_node*>* s, conflict_overlap_data* d){
...
void remove_conflicts(set<apta_node*>* s, conflict_overlap_data* d){
    set<apta_node*>::iterator it  = s->begin();
    set<apta_node*>::iterator it2 = d->conflicts.begin();
...
bool full_overlap(conflict_overlap_data* d, conflict_overlap_data* d2){
...
int count_overlap(set<apta_node*>* s, conflict_overlap_data* d){
    set<apta_node*>::iterator it  = s->begin();
    set<apta_node*>::iterator it2 = d->conflicts.begin();
...
void conflict_overlap_driven::initialize(state_merger *merger){
    ...
    for(merged_APTA_iterator it = merged_APTA_iterator(merger->aut->root); *it != 0; ++it){
        ...
        for(merged_APTA_iterator it2 = it; *it2 != 0; ++it2){
            ...
            if(merger->merge_test(n1,n2) == false){
                conflict_overlap_data* d1 = (conflict_overlap_data*)n1->data;
                conflict_overlap_data* d2 = (conflict_overlap_data*)n2->data;
    ...
    for(merged_APTA_iterator it = merged_APTA_iterator(merger->aut->root); *it != 0; ++it){
        conflict_overlap_data* d = (conflict_overlap_data*)(*it)->data;
    ...
    for(set<apta_node*, conflict_compare>::reverse_iterator it = all_nodes.rbegin(); it != all_nodes.rend(); ++it){
        apta_node* n = *it;
        conflict_overlap_data* d = (conflict_overlap_data*)n->data;
        if(d->conflicts.size() < 50) continue;
        ...
        if(bip_nodes.size() > 50){
            ...
            for(set<apta_node*>::iterator itl = bip_nodes.begin(); itl != bip_nodes.end(); ++itl){
                apta_node* l = *itl;
                conflict_overlap_data* dl = (conflict_overlap_data*)l->data;
                for(set<apta_node*>::iterator itr = bip2_nodes.begin(); itr != bip2_nodes.end(); ++itr){
                    apta_node* r = *itr;
                    conflict_overlap_data* dr = (conflict_overlap_data*)r->data;
    ...
    while(true){
        ...
        for(set<apta_node*, conflict_compare>::reverse_iterator it = all_nodes.rbegin(); it != all_nodes.rend(); ++it){
            ...
            if(itl != left.end()){
                cerr << "l" << count_overlap(&right, ((conflict_overlap_data*)n->data)) << endl;
                //if(count_overlap(&right, ((conflict_overlap_data*)n->data)) > 0.1 * right.size()){
                if(full_overlap(&right, ((conflict_overlap_data*)n->data))){
                    remove_conflicts(&left, ((conflict_overlap_data*)n->data));
                    //keep_conflicts(&right, ((conflict_overlap_data*)n->data));
                } else {
                    left.erase(itl);
                }
            } else if(itr != right.end()){
                cerr << "r" << count_overlap(&left, ((conflict_overlap_data*)n->data)) << endl;
                //if(count_overlap(&left, ((conflict_overlap_data*)n->data)) > 0.1 * left.size()){
                if(full_overlap(&left, ((conflict_overlap_data*)n->data))){
                    remove_conflicts(&right, ((conflict_overlap_data*)n->data));
                    //keep_conflicts(&left, ((conflict_overlap_data*)n->data));
                } else {
                    right.erase(itr);
                }
            }
